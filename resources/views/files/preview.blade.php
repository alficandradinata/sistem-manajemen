<x-app-layout :title="$file->original_name">
    ...
    <div class="card mb-4 flex items-center justify-between gap-3 px-4 py-3">
        <div class="min-w-0">
            <p class="truncate font-medium text-stone-900">{{ $file->original_name }}</p>
            <p class="mt-0.5 text-xs text-stone-400">{{ $file->category_label }} &middot; {{ $file->readable_size }}</p>
        </div>
    ...
    @elseif ($file->preview_kind === 'pdf')
        <object data="{{ route('files.raw', $file) }}" type="application/pdf"
                class="h-[70vh] w-full rounded-xl border border-stone-200 bg-white">
                ...
                <table class="min-w-full text-sm">
                    <tbody>
                        @foreach ($rows as $r => $row)
                            <tr class="border-b border-stone-100 last:border-0 {{ $r === 0 ? 'bg-stone-50 font-medium text-stone-900' : '' }}">
                                @foreach ($row as $cell)
                                    <td class="px-3 py-2 whitespace-nowrap text-stone-700">{{ $cell }}</td>
                                @endforeach
                            </tr>
                        @endforeach
                    </tbody>
    ...
    @elseif ($file->preview_kind === 'document')
            ...
            <div class="card-pad space-y-3">
                @foreach ($blocks as $block)
                    @if ($block['type'] === 'paragraph')
                        <p class="text-sm leading-relaxed text-stone-700">{{ $block['text'] }}</p>
                    @else
                                ...
                                <tbody>
                                    @foreach ($block['rows'] as $row)
                                        <tr class="border-b border-stone-100 last:border-0">
                                            @foreach ($row as $cell)
                                                <td class="px-3 py-2 text-stone-700">{{ $cell }}</td>
                                            @endforeach
                                        </tr>
                                    @endforeach
                                </tbody>
        ...
        <p class="mt-2.5 text-xs text-stone-400">Tampilan teks saja — tata letak dan gambar tidak ikut terbawa.</p>

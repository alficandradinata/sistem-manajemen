<x-app-layout :title="$file->original_name">
    ...
    <div class="card mb-4 flex items-center justify-between gap-3 px-4 py-3">
        <div class="min-w-0">
            ...
            <p class="mt-0.5 text-xs text-slate-400">{{ $file->category_label }} &middot; {{ $file->readable_size }}</p>
        </div>
                ...
                <table class="min-w-full text-sm">
                    <tbody>
                        @foreach ($rows as $r => $row)
                            <tr class="border-b border-slate-100 last:border-0 {{ $r === 0 ? 'bg-slate-50 font-medium text-slate-900' : '' }}">
                                @foreach ($row as $cell)
    ...
    @elseif ($file->preview_kind === 'document')
        ...
        <p class="mt-2.5 text-xs text-slate-400">Tampilan teks saja — tata letak dan gambar tidak ikut terbawa.</p>

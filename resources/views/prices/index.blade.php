<x-app-layout title="Daftar Harga">
    ...
    <details class="card mb-5"
        ...
        <summary class="flex cursor-pointer list-none items-center gap-2 px-4 py-3 text-sm font-medium text-slate-900 dark:text-slate-100">
            <svg class="size-4 text-slate-400 dark:text-slate-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.25" stroke-linecap="round">
                <path d="M12 5v14M5 12h14"/>
            </svg>
            Tambah harga
        </summary>

        <form method="POST" action="{{ route('prices.store') }}" class="border-t border-slate-100 p-4 dark:border-slate-800">
            @csrf
    ...
    @if ($prices->isNotEmpty())
        <div class="card overflow-hidden">
            @foreach ($prices as $price)
                <details class="group border-b border-slate-100 last:border-0 dark:border-slate-800">
                    <summary class="flex cursor-pointer list-none items-center justify-between gap-3 px-4 py-3 hover:bg-slate-50 dark:hover:bg-slate-800">
                        <span class="min-w-0 truncate text-sm text-slate-800 dark:text-slate-200">{{ $price->name }}</span>
                        <span class="flex shrink-0 items-center gap-2">
                            <span class="text-sm font-medium text-slate-900 dark:text-slate-100">
                                Rp{{ number_format($price->price, 0, ',', '.') }}
                            </span>
                            <svg class="size-4 text-slate-400 transition-transform group-open:rotate-180 dark:text-slate-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round">
                                <path d="m6 9 6 6 6-6"/>
                            </svg>
                        </span>
                    </summary>

                    <div class="border-t border-slate-100 bg-slate-50/60 p-4 dark:border-slate-800 dark:bg-slate-800/40">

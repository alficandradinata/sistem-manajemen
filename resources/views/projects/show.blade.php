<x-app-layout :title="$project->name">
    ...
    <div class="card-pad mb-6">
        <div class="flex items-start justify-between gap-3">
            <div class="min-w-0">
                <h1 class="text-xl font-semibold tracking-tight text-stone-900">{{ $project->name }}</h1>
                <p class="mt-1.5 flex items-center gap-1.5 text-sm text-stone-500">
                    <svg class="size-3.5 text-stone-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round">
    ...
    <form method="POST" action="{{ route('projects.files.store', $project) }}" enctype="multipart/form-data"
        ...
        <input id="files" name="files[]" type="file" multiple required
               ...
               class="mb-4 block w-full text-sm text-stone-600 file:mr-3 file:rounded-lg file:border-0 file:bg-stone-900 file:px-4 file:py-2.5 file:text-sm file:font-medium file:text-white hover:file:bg-stone-700">
    ...
    <form method="POST" action="{{ route('projects.files.drive', $project) }}" class="card-pad mb-6">
        ...
        <p class="field-hint">
            ...
            <span class="font-medium text-stone-700">Ambil file</span> menyalinnya ke sini agar bisa dipratinjau;
            <span class="font-medium text-stone-700">Simpan link</span> hanya mencatat alamatnya — pilih itu untuk
    ...
    @foreach ($categoryLabels as $key => $label)
        ...
        <section class="card mb-2 overflow-hidden">
            <header class="flex items-center justify-between border-b border-stone-100 px-4 py-2.5">
                <h3 class="text-sm font-medium text-stone-900">{{ $label }}</h3>
                <span class="text-xs text-stone-400">{{ $files->count() }}</span>
            </header>

            @forelse ($files as $file)
                <div class="flex items-center gap-3 border-b border-stone-100 px-4 py-3 last:border-0">
                    @if ($file->preview_kind === 'image')
                        <a href="{{ route('files.preview', $file) }}" class="shrink-0">
                            <img src="{{ route('files.raw', $file) }}" alt=""
                                 class="size-10 rounded-md border border-stone-200 bg-stone-100 object-cover">
                        </a>
                    @endif

                    <div class="min-w-0 flex-1">
                        <p class="truncate text-sm text-stone-800">{{ $file->original_name }}</p>
                        <p class="mt-0.5 flex items-center gap-1.5 text-xs text-stone-400">
                            @if ($file->size)
                                <span>{{ $file->readable_size }}</span>
                            @endif
                            @if ($file->is_link)
                                <span class="inline-flex items-center gap-1 rounded border border-stone-200 px-1.5 py-px text-[11px] text-stone-500">
            ...
            @empty
                <p class="px-4 py-3 text-sm text-stone-400">Belum ada file.</p>
            @endforelse
        </section>
    @endforeach
</x-app-layout>
